...
const Navbar = ({ toggleSidebar }) => {
    ...
    return (
        <nav className="glass-effect border-b border-white/20 dark:border-gray-700 h-16 flex items-center justify-between px-6 fixed w-full top-0 z-30 transition-colors duration-300">
            <div className="flex items-center gap-4">
                <button
                    onClick={toggleSidebar}
                    className="p-2 hover:bg-white/10 dark:hover:bg-gray-800 rounded-xl lg:hidden transition-colors"
                >
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="relative" ref={notifRef}>
                    <button
                        ...
                        className="p-2.5 hover:bg-white/50 dark:hover:bg-gray-800 rounded-xl relative transition-all duration-200 hover:scale-105"
                    >
                    ...
                    {showNotifications && (
                        <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 py-2 animate-scale-in origin-top-right">
                            ...
                            <div className="max-h-96 overflow-y-auto">
                                {notifications.map(notif => (
                                    <div key={notif.id} className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer ${!notif.read ? 'bg-teal-50/30 dark:bg-teal-900/10' : ''}`}>
                                        <div className="flex gap-3">
                                            <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${!notif.read ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}></div>
                                            <div>
                                                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{notif.title}</p>
                                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{notif.text}</p>
                ...
                <div className="relative" ref={profileRef}>
                    <button
                        ...
                        className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-xl hover:bg-white/50 dark:hover:bg-gray-800 transition-all"
                    >
                    ...
                    {showProfileMenu && (
                        <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 py-2 animate-scale-in origin-top-right">
